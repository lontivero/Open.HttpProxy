...
namespace Open.Tcp.BufferManager
{
    public sealed class ManualBufferedStream : Stream {
        ...
        private ArraySegment<byte> _buffer;    // Shared read/write buffer.  Alloc on first use.
        ...
        private const int _DefaultBufferSize = BufferAllocator.BlockSize;
 
        public ManualBufferedStream(Stream stream, BufferAllocator allocator) 
            : this(stream, allocator, _DefaultBufferSize)
        {
        }
        ...
        private async Task FlushWriteAsync() {
            await _s.WriteAsync(_buffer.Array,  _buffer.Offset, _buffer.Offset + _writePos);
            _writePos = 0;
            await _s.FlushAsync();
        }
